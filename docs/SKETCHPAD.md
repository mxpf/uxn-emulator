...
is one operation; holding a key does not repeat. There is no save format, toolbar,
undo history, animation clock or in-app recorder. Closing/reloading loses the
sheet. The amber cursor is an overlay, not part of the drawing.
...
295-byte ROM, one explicitly declared external input route (selector 07), and
one explicitly attached display callback. There are no inter-node messages
...
action bytes 1–6. Neither C nor JavaScript changes this guest state to draw.
...
- 426 native/WebAssembly checkpoints compare canonical fingerprints of full
...
- Real-browser checks cover six buttons, keyboard input, focused-button
  Space/Enter behavior, ignored key repeats, tactile depression, four viewport
